...
```
git clone git@github.com:Gomah/prisma-negative-cursor-repro.git
```
...
```gql
query {
  users(where: { email: { equals: "jack@prisma.io" } }) {
    posts(orderBy: { id: asc }, take: 10, cursor: { id: 40 }) {
      id
    }
  }
}
...
This should return 10 posts from `ID` 40 to 49:
...
However, the -10 doesn't seem to work:

```gql
query {
  users(where: { email: { equals: "jack@prisma.io" } }) {
    posts(orderBy: { id: asc }, take: -10, cursor: { id: 40 }, skip: 1) {
      id
    }
  }
}
...
It should return 10 posts from `ID` 30 to 39 but instead, it's just returning the last 10.
